...
class Pos_invoices extends CI_Controller
{
    ...
    public function create()
    {
        ...
        $data['lastinvoice_v2_new'] = $this->invocies->lastinvoice_v2_new();
    ...
    public function quotecreate()
    {
        ...
        $data['enable_card'] = $this->plugins->universal_api(54);
        $data['customergrouplist'] = $this->customers->group_list();
        $data['lastinvoice'] = $this->invocies->lastinvoice1();
        $data['draft_list'] = $this->invocies->drafts();
        $data['warehouse'] = $this->invocies->warehouses();
        $data['terms'] = $this->invocies->billingterms();
        $data['currency'] = $this->invocies->currencies();
        ...
        if ($this->input->get('v2') OR POSV == 2) {
            $head['s_mode'] = false;
            $this->load->view('fixed/header-pos', $head);
            $this->load->view('pos/quotecreate', $data);
        } else {
            $head['s_mode'] = true;
            $this->load->view('fixed/header-pos', $head);
            $this->load->view('pos/quotecreate', $data);
    ...
    public function purchaseedit()
    {
        ...
        $this->load->model('customers_model', 'customers');
        $data['customergrouplist'] = $this->customers->group_list();
        $data['terms'] = $this->invocies->purchase_billingterms();
        $data['currency'] = $this->invocies->currencies();
        $data['invoice'] = $this->invocies->purchase_details($tid, $this->limited); 
        if ($data['invoice']['id']) $data['products'] = $this->invocies->purchase_products($tid);
        $head['title'] = "Edit Purchase #$tid";
        $head['usernm'] = $this->aauth->get_user()->username;
        $data['warehouse'] = $this->invocies->warehouses();
        $this->load->model('plugins_model', 'plugins');
        $data['exchange'] = $this->plugins->universal_api(5);
        ...
        $this->load->view('fixed/header-pos', $head);
        if ($data['invoice']['id']) $this->load->view('pos/purchaseedit', $data);
        $this->load->view('fixed/footer-pos');
    ...
    public function purchasecreate()
    {
        ...
        if ($this->input->get('v2') OR POSV == 2) {
            $head['s_mode'] = false;
            $this->load->view('fixed/header-pos', $head);
            $this->load->view('pos/purchasecreate', $data);
        } else {
            $head['s_mode'] = true;
            $this->load->view('fixed/header-pos', $head);
            $this->load->view('pos/purchasecreate', $data);
    ...
    public function edit()
    {
     ...
     //invoices_list_action
    public function invoices_list_action()
    {
        $head['title'] = "SAW MILLS & TIMBER STORES -- Tax Monthly Summary -- VAT ID 662941794-0000";
        $head['usernm'] = $this->aauth->get_user()->username;
        $this->load->view('fixed/header', $head);
        $this->load->view('pos/invoices_list_action');
        $this->load->view('fixed/footer');
    }


     //export_pos_invoices_v2
    public function export_pos_invoices_v2()
    {
        $price_second = intval($this->input->post('price_second'));
        ...
        if ($price_second) {
            ...
            $this->db->where('DATE(invoicedate) >=', date('Y-m-d', strtotime($s_date)));
            $this->db->where('DATE(invoicedate) <=', date('Y-m-d', strtotime($e_date)));
            ...
            $html = $this->load->view('pos/export_pos_invoicespdf-ltr', array('report' => $result,'price_second' => $price_second), true);
            ...
            $head['title'] = "Export POS Invoices";
            $head['usernm'] = $this->aauth->get_user()->username;
            $this->load->view('fixed/header', $head);
            $this->load->view('pos/export_pos_invoices');
            $this->load->view('fixed/footer');
        }
    }

     //export_pos_invoices
    public function export_pos_invoices()
    {
        $price_second = intval($this->input->post('price_second'));
    ...
    public function action()
    {
        ...
        if ($i_class == '1') {
            $last = $this->invocies->lastinvoice();
            $invocieno = $last + 1;
        } else {
            $last = $this->invocies->lastinvoice_v2_new();
            $invocieno = $last + 1;
        }
...
if (isset($local_imported[$key])) {
    ...
    if ($local_imported[$key] == 'l') {
        ...
        $i_subtotal += $converted_amount;
    }
}

// After the loop, calculate SSC
$l_subtotal80 = $l_subtotal * 0.85; // 85% of local subtotal
$invoices_ssc_imported = $i_subtotal * 0.025; // 2.5% of imported subtotal
$invoices_ssc_local = $l_subtotal80 * 0.025; // 2.5% of 85% of local subtotal
    ...
    $data = array(
        ...
        'qty2' => $product_qty2[$key]    
              ...
              // $amt2 = numberClean($product_qty[$key]);         
                    ...
                    $this->db->set(array('local' => $invoices_ssc_local, 'imported' => $invoices_ssc_imported,));
                ...
                if ($transok) {
                    ...
                    // <a target='_blank' href='thermal_pdf?id=$invocieno' class='btn btn-blue btn-lg'><span class='fa fa-ticket' aria-hidden='true'></span> PDF  </a> &nbsp; &nbsp;   <a id='$p_tid' data-ptid='$invocieno' data-url='" . $printer['val3'] . "'  class='btn btn-info btn-lg white'><span class='fa fa-ticket' aria-hidden='true'></span> " . $this->lang->line('Thermal Printer') . "  </a> &nbsp; &nbsp;
                    
                    
                        $this->lang->line('Invoice Success') . "  <a target='_blank' href='printinvoice?id=$invocieno&id2=' class='btn btn-blue btn-lg'><span class='fa fa-print' aria-hidden='true'></span> A4  </a> &nbsp; &nbsp; <a href='view?id=$invocieno&id2=' class='btn btn-purple btn-lg'><span class='fa fa-eye' aria-hidden='true'></span> " . $this->lang->line('View') . "  </a> &nbsp; &nbsp; <a href='$link&id2=' class='btn btn-blue-grey btn-lg'><span class='fa fa-globe' aria-hidden='true'></span> " . $this->lang->line('Public View') . " </a> &nbsp;<a href='create' class='btn btn-flickr btn-lg'><span class='fa fa-plus-circle' aria-hidden='true'></span> " . $this->lang->line('Create') . "  </a>"));
                }
...
// Your Account SID and Auth Token from twilio.com/console
        ...
        //profit calculation 
        $t_profit = 0;
        $this->db->select('geopos_invoice_items.pid, geopos_invoice_items.subtotal, geopos_invoice_items.qty, geopos_products.fproduct_cost');
        ...
        foreach ($pids as $profit) {
            $t_cost = $profit['fproduct_cost'] * $profit['qty'];
            $s_cost = $profit['subtotal'];
 ...
 public function ajax_list_price_second()
    {
       $price_second = $this->input->post('price_second');
        ...
        foreach ($list as $invoices) {
                  ...
                  if ($invoicetax > 0) {
                   ...
                   $invoice_tota_oklk     =      $invoices->total;
                    ...
                    $invoice_tota_oklk_2 = ($percentage / 100) * $invoice_tota_oklk;
                      ...
                      $total_due     =      $invoices->total - $invoices->pamnt;

                    $total_due_oklk_2 = ($percentage / 100) * $total_due;
                    
                    
                     $invoices_pamnt     =      $invoices->pamnt;

                    $invoice_pamnt_oklk_2 = ($percentage / 100) * $invoices_pamnt;
              
              $invoices_ssc_imported = ($percentage / 100) * $invoices->imported;
              $invoices_ssc_local = ($percentage / 100) * $invoices->local;             
            ...
            $row[] = amountExchange($invoices_tax);
        ...
        //$row[] = amountExchange($invoices_tax);
       // $row[] = amountExchange($invoices_ssc);
       // $row[] = amountExchange($invoices_without_tax);
        
          //  $row[] = '<span class="st-' . $invoices->status . '">' . $this->lang->line(ucwords($invoices->status)) . '</span>';
            $row[] = '<a target="_blank"  href="' . base_url("pos_invoices/printinvoice?id=$invoices->id&is=$price_second") . '" class="btn btn-success btn-sm" title="View"><i class="fa fa-eye"></i></a>&nbsp;';
    ...
    public function ajax_list_price_second_v2()
    {
       $price_second = $this->input->post('price_second');

        $list = $this->invocies->get_datatables_v2($this->limited);
       ...
       // $row[] = amountExchange($invoices_without_tax);
        ...
        $output = array(
            "draw" => $this->input->post('draw'),
            "recordsTotal" => $this->invocies->count_all_v2($this->limited),
            "recordsFiltered" => $this->invocies->count_filtered_v2($this->limited),
            "data" => $data,
        );
        ...
        foreach ($list as $invoices) {
             ...
             $admin2019 =    $this->aauth->get_user()->id;
              ...
              if($admin2019==10){
                  ...
                  $invoice_tota_oklk_2     =      $invoices->total;
                  $total_due_oklk_2     =      $invoices->total - $invoices->pamnt;
                   $invoice_pamnt_oklk_2     =      $invoices->pamnt;
    ...
    public function purchase_printinvoice()
    {
        $tid = $this->input->get('id');
        $data['id'] = $tid;
        $data['round_off'] = $this->custom->api_config(4);
        ...
        if ($data['invoice']['id']) $data['products'] = $this->invocies->purchase_products($tid);
        if ($data['invoice']['id']) $data['employee'] = $this->invocies->employee($data['invoice']['eid']);
        if ($data['invoice']['id']) $data['activity'] = $this->invocies->purchase_transactions($tid);
        if (CUSTOM) $data['c_custom_fields'] = $this->custom->view_fields_data($data['invoice']['cid'], 1, 1);
        if ($data['invoice']['i_class'] == 1) {
            $pref = prefix(7);
        } else {
   ...
   public function editaction()
    {
        ...
        $invocieduedate = $this->input->post('invocieduedate');
        $currency = $this->input->post('mcurrency');
        $i_class = $this->input->post('i_class') ?: 0; // Default to 0 if not set
        $loadingunloading = $this->input->post('loadingunloading');
        $cuttingsawing = $this->input->post('cuttingsawing');
        $planing = $this->input->post('planing');
        
        // Initialize tax variables to prevent undefined variable warnings
        $planing_tax = 0;
        $planing_taxtype = 'off';
        $cuttingsawing_tax = 0;
        ...
        if ($ptype == 4) {
            ...
            $data = array('invoicedate' => $bill_date, 'invoiceduedate' => $bill_due_date, 'subtotal' => $subtotal, 'shipping' => $shipping, 'ship_tax' => $shipping_tax, 'ship_tax_type' => $ship_taxtype, 'planing' => $planing, 'planing_tax' => $planing_tax, 'planing_taxtype' => $planing_taxtype, 'cuttingsawing' => $cuttingsawing, 'cuttingsawing_tax' => $cuttingsawing_tax, 'cuttingsawing_taxtype' => $cuttingsawing_taxtype,  'loadingunloading' => $loadingunloading, 'loadingunloading_tax' => $loadingunloading_tax, 'loadingunloading_taxtype' => $loadingunloading_taxtype, 'total' => $total, 'notes' => $notes, 'csd' => $customer_id, 'taxstatus' => $tax, 'discstatus' => $discstatus, 'format_discount' => $discountFormat, 'refer' => $refer, 'term' => $pterms, 'multi' => $currency);
            $this->db->set($data);
            $this->db->where('id', $invocieno);
            if ($this->db->update('geopos_invoices', $data)) {
                ...
                foreach ($pid as $key => $value) {
                    ...
                    $data = array(
                        ...
                        'pwith' => $product_pwith[$key],
                        'pthickness' => $product_thicknes[$key],
                        'pquick' => numberClean($o_pquick[$key]),
                        'pquick_code' => $product_quick_code[$key],
                        'local_imported' => $local_imported[$key],
                        'o_pquick' => $o_pquick[$key],
                        'sqft' => $product_sqft[$key],
                    ...
                    if ($local_imported[$key] == 'l') {
    $l_subtotal = $product_price[$key];
} elseif ($local_imported[$key] == 'i') {
    $i_subtotal = $product_price[$key];
}
                    ...
                    $amt = numberClean($pquick[$key]) - numberClean($old_product_pquick[$key]);
                  //  if ($product_id[$key] > 0) {
                  //      $this->db->set('qty', "qty-$amt", FALSE);
                  //      $this->db->where('product_code', $product_quick_code[$key]);
                   //     $this->db->update('geopos_products');
                   // }
                    ...
                    $amt2 = numberClean($pquick[$key]) - numberClean($old_product_pquick[$key]);
                ...
                if ($prodindex > 0) {
                    ...
                    $this->db->set(array('local' => $l_subtotal, 'imported' => $i_subtotal,));
                    $this->db->where('id', $invocieno);
                    $this->db->update('geopos_invoices');
...
// Handle stock reversal for removed items
// DEBUG: Log what we're receiving
error_log("=== RESTOCK DEBUG ===");
error_log("POST restock: " . print_r($this->input->post('restock'), true));
error_log("POST restock_cubic_3: " . print_r($this->input->post('restock_cubic_3'), true));
error_log("POST restock_cubic: " . print_r($this->input->post('restock_cubic'), true));

$restock_items = $this->input->post('restock');
if (!empty($restock_items)) {
    error_log("Restock items is NOT empty");
    error_log("Is array: " . (is_array($restock_items) ? 'YES' : 'NO'));
    
    // Ensure it's always an array, even if single element
    if (!is_array($restock_items)) {
        $restock_items = array($restock_items);
        error_log("Converted to array");
    }
    
    error_log("Processing " . count($restock_items) . " restock items");
    
    foreach ($restock_items as $value) {
        error_log("Processing restock value: " . $value);
        $myArray = explode('-', $value);
        if (count($myArray) >= 2) {
            $prid = $myArray[0];
            $dqty = numberClean($myArray[1]);
            error_log("  PID: $prid, Qty to restore: $dqty");
            if (!empty($prid) && $dqty > 0) {
                $this->db->set('qty', "qty+$dqty", FALSE);
                $this->db->where('pid', $prid);
                $this->db->update('geopos_products');
                error_log("  Updated qty for PID $prid");
            }
        }
    }
} else {
    error_log("Restock items is EMPTY or NULL");
}

$restock_cubic_3_items = $this->input->post('restock_cubic_3');
if (!empty($restock_cubic_3_items)) {
    // Ensure it's always an array, even if single element
    if (!is_array($restock_cubic_3_items)) {
        $restock_cubic_3_items = array($restock_cubic_3_items);
    }
    
    foreach ($restock_cubic_3_items as $value) {
        $myArray = explode('-', $value);
        if (count($myArray) >= 2) {
            $prid = $myArray[0];
...
$restock_cubic_items = $this->input->post('restock_cubic');
if (!empty($restock_cubic_items)) {
    // Ensure it's always an array, even if single element
    if (!is_array($restock_cubic_items)) {
        $restock_cubic_items = array($restock_cubic_items);
    }
    
    foreach ($restock_cubic_items as $value) {
        $myArray = explode('-', $value);
        if (count($myArray) >= 2) {
            $pidqcode = $myArray[0];
            $pdinpquick = numberClean($myArray[1]);
...
error_log("=== END RESTOCK DEBUG ===");
        ...
        //profit calculation
        $t_profit = 0;
        $this->db->select('geopos_invoice_items.pid, geopos_invoice_items.price, geopos_invoice_items.qty, geopos_products.fproduct_price, geopos_products.fproduct_cost, geopos_invoice_items.subtotal');
        $this->db->from('geopos_invoice_items');
        $this->db->join('geopos_products', 'geopos_products.pid = geopos_invoice_items.pid', 'left');
        $this->db->where('geopos_invoice_items.tid', $invocieno);
        $query = $this->db->get();
