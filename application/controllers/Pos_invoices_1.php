...
class Pos_invoices extends CI_Controller
{
    ...
    {
        $head['title'] = "HAPUARACHCHI SAW MILLS & TIMBER STORES -- Tax Monthly Summary -- VAT ID 662941794-7000";
        $head['usernm'] = $this->aauth->get_user()->username;
    ...
    public function action()
    {
       ...
       if ($pquick[$key] > 0) {               
                     $amt = numberClean($product_qty[$key] * $pquick[$key]);                    
                    if ($product_id[$key] > 0) {
 ...
 $amt = 0;                   
            ...
            $data = array('tid' => $invocieno, 'invoicedate' => $bill_date, 'invoiceduedate' => $bill_due_date, 'subtotal' => $subtotal, 'shipping' => $shipping, 'planing' => $planing,  'cuttingsawing' => $cuttingsawing,  'loadingunloading' => $loadingunloading, 'total' => $total, 'notes' => $notes, 'csd' => $customer_id, 'eid' => $this->aauth->get_user()->id, 'taxstatus' => $tax, 'discstatus' => $discstatus, 'format_discount' => $discountFormat, 'refer' => $refer, 'term' => $pterms, 'multi' => $currency, 'i_class' => 1, 'loc' => $this->aauth->get_user()->loc);
            $invoice_true = false;
            if ($this->db->insert('geopos_invoices', $data)) {
                $invoice_true = true;
                $tid = $invocieno;
 ...
 public function ajax_list_price_second()
    {
       $price_second = $this->input->post('price_second');
        ...
        foreach ($list as $invoices) {
              ...
              $invoices_ssc = ($invoice_tota_oklk_2 /100*2.5);
