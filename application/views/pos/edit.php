<style>
/* POS Product Card Design - Copied from newinvoice_v2.php */
.pos-product-card {
    border: none;
            ...
            <div id="saman-row-pos" class="rqw mt-1">
                <div class="col p-mobile">
                    ...
                    <table id="pos_list" class="table-responsive tfr pos_stripe">
                        ...
                        <tbody id="pos_items">
                        <!-- CHANGED: Insert PHP Loop from edit.php with newinvoice_v2 structure -->
                        <?php $i = 0;
                        foreach ($products as $row) {
                            ...
                            $price = $row['price'];
                            $unit = $row['unit'];
                            $product_des = is_numeric($row['product_des']) ? $row['product_des'] : 0;
                            $sqft_db = isset($row['sqft']) ? $row['sqft'] : 0;
                            $pwith = isset($row['pwith']) ? $row['pwith'] : 0;
                            $subtotal = 0;

                            if ($unit == 'Sqft') {
                                if ($sqft_db > 0) {
                                    $subtotal = $rate * $qty * $sqft_db;
                                } else {
                                    $width_ft = $pwith / 12;
                                    $subtotal = $rate * $qty * ($width_ft * $product_des);
                                }
                            } elseif ($unit == 'Feet') {
                                $subtotal = $rate * $qty * $product_des;
                            } else {
                               if ($product_des > 0) {
                                    $subtotal = $price * $qty * $product_des;
                                } else {
                                    $subtotal = $price * $qty;
                                }
                            }
                            if ($subtotal == 0 && isset($row['subtotal']) && $row['subtotal'] > 0) {
                                $subtotal = $row['subtotal'];
                            }
                            $subtotal_fmt = amountExchange_s($subtotal, $invoice['multi'], $this->aauth->get_user()->loc);
                            
                            $functionNum = "'" . $i . "'";

                            $unit_val = strtolower(trim($row['unit']));
                            
                            // DEBUG (Commented out)
                            // echo "<pre>"; var_dump($row); echo "</pre>";

                            $rate_val = isset($row['rate']) ? amountExchange_s($row['rate'], $invoice['multi'], $this->aauth->get_user()->loc) : amountExchange_s($row['price'], $invoice['multi'], $this->aauth->get_user()->loc);

                            if (!empty($unit_val) && ($unit_val == 'feet' || $unit_val == 'sqft' || $unit_val == 'sq.ft')) {
                                $input_html = '<input type="text" class="form-control p-mobile req prc" name="product_rate[]" id="rate-' . $i . '" onkeypress="return isNumber(event)" onkeyup="rowTotal(' . $functionNum . '), billUpyog()" autocomplete="off" value="' . $rate_val . '"><input type="hidden" name="product_price[]" id="price-' . $i . '" value="' . amountFormat_general($row['price']) . '">';
                            } else {
                                $input_html = '<input type="text" class="form-control p-mobile req prc" name="product_price[]" id="price-' . $i . '" onkeypress="return isNumber(event)" onkeyup="rowTotal(' . $functionNum . '), billUpyog()" autocomplete="off" value="' . amountExchange_s($row['price'], $invoice['multi'], $this->aauth->get_user()->loc) . '"><input type="hidden" name="product_rate[]" id="rate-' . $i . '" value="' . amountFormat_general($row['rate']) . '">';
                            }

                            echo '<tr id="ppid-' . $i . '" class="mb-1">
                                <td class="p-1">
                                    <input type="text" class="form-control text-left p-mobile product-name-input" style="font-weight: 600; color: #333;" name="product_name[]" placeholder="Enter Product name or Code" id="productname-' . $i . '" value="' . $row['product'] . '">
                                    <input type="hidden" id="alert-' . $i . '" value="0.00" name="alert[]">
                                </td>
                                <td>
                                    <input type="text" class="form-control p-mobile req amnt" name="product_qty[]" id="amount-' . $i . '" onkeypress="return isNumber(event)" onkeyup="rowTotal(' . $functionNum . '), billUpyog()" autocomplete="off" value="' . amountFormat_general($row['qty']) . '">
                                    <input type="hidden" class="old_amnt" name="old_product_qty[]" value="' . amountFormat_general($row['qty']) . '">
                                </td>
                                <td>
                                    ' . $input_html . '
                                </td>
                                <td>
                                    <input type="text" class="form-control p-mobile vat" name="product_tax[]" id="vat-' . $i . '" onkeypress="return isNumber(event)" onkeyup="rowTotal(' . $functionNum . '), billUpyog()" autocomplete="off" value="' . amountFormat_general($row['tax']) . '">
                                </td>
                                <td>
                                    <input type="text" class="form-control p-mobile discount pos_w" name="product_discount[]" onkeypress="return isNumber(event)" id="discount-' . $i . '" onkeyup="rowTotal(' . $functionNum . '), billUpyog()" autocomplete="off" value="' . amountFormat_general($row['discount']) . '">
                                </td>
                                <td>
                                    <span class="currenty">' . $this->config->item('currency') . '</span> <strong><span class="ttlText" id="result-' . $i . '">' . $subtotal_fmt . '</span></strong><br>
                                    <span class="planingText" id="resultplaning2-' . $i . '">Planing: 0</span>
                                    <input class="planing2" type="checkbox" id="planing2-' . $i . '" name="planing2" style="display:none;">
                                </td>
                                <td class="text-center">
                                    <div class="btn-group btn-group-sm" role="group">
                                        <button type="button" class="btn btn-danger removeItem" data-rowid="' . $i . '" title="Remove"> <i class="fa fa-trash"></i> </button>
                                        <button type="button" class="btn btn-warning toggle-planing" data-rowid="' . $i . '" title="Toggle Planing"> <i class="fa fa-layers"></i> </button>
                                    </div>
                                </td>
                                ...
                                <input type="hidden" class="ttInput" name="product_subtotal[]" id="total-' . $i . '" value="' . $subtotal_fmt . '">
                                <input type="hidden" class="pdIn" name="pid[]" id="pid-' . $i . '" value="' . $row['pid'] . '">
                                <input type="hidden" class="pdInqcode" name="pdInqcode[]" id="pdInqcode-' . $i . '" value="' . $row['pquick_code'] . '">
                                <input type="hidden" class="pdInpquick" name="pquick[]" id="pquick-' . $i . '" value="' . $row['pquick'] . '">
                                <input type="hidden" name="unit[]" id="unit-' . $i . '" value="' . $row['unit'] . '">
                                <input type="hidden" name="hsn[]" id="hsn-' . $i . '" value="' . $row['code'] . '">
                                <input type="hidden" class="form-control description" name="product_description[]" id="product_description-' . $i . '" value="' . $row['product_des'] . '">
                                <input type="hidden" name="product_pwith[]" id="product_pwith-' . $i . '" value="' . $row['pwith'] . '">
                                <input type="hidden" name="product_thicknes[]" id="product_thicknes-' . $i . '" value="' . $row['pthickness'] . '">
                                <input type="hidden" name="old_product_pquick[]" id="pquick-' . $i . '" value="' . $row['pquick'] . '">
                                <input type="hidden" class="product_quick_code" name="product_quick_code[]" id="product_quick_code-' . $i . '" value="' . $row['pquick_code'] . '">
                                <input type="hidden" name="local_imported[]" id="local_imported-' . $i . '" value="' . $row['local_imported'] . '">
                                <input type="hidden" class="o_pquick" name="o_pquick[]" id="o_pquick-' . $i . '" value="' . ($row['o_pquick']) . '">
                                <input type="hidden" name="qty2[]" id="qty2-' . $i . '" value="' . (isset($row['qty2']) ? $row['qty2'] : 0) . '">
                                <input type="hidden" name="product_sqft[]" id="sqft-' . $i . '" value="' . (isset($row['sqft']) ? $row['sqft'] : 0) . '">
                                <input type="hidden" name="group_id[]" id="group_id-' . $i . '" value="' . (isset($row['group_id']) ? $row['group_id'] : 0) . '">
                            </tr>';
                            $i++;
                        } ?>
                        </tbody>
                                    ...
                                    <div class="row no-gutters align-items-center">
                                         <div class="col-md-3 pr-1">
                                            <div class="input-group input-group-sm">
                                                ...
                                                <input type="text" class="form-control form-control-sm cuttingsawingVal font-weight-bold" name="cuttingsawing" placeholder="0" onkeypress="return isNumber(event)" onkeyup="billUpyog()" value="<?php echo $invoice['cuttingsawing']; ?>">
                                            </div>
                                         </div>
                                         <div class="col-md-3 pr-1">
                                             <div class="input-group input-group-sm">
                                         ...
                                         <div class="col-md-3 pr-1">
                                             <div class="input-group input-group-sm">
                                                ...
                                                <input type="text" class="form-control form-control-sm loadingunloadingVal font-weight-bold" name="loadingunloading" placeholder="0" onkeypress="return isNumber(event)" onkeyup="billUpyog()" value="<?php echo $invoice['loadingunloading']; ?>">
                                            </div>
                                         </div>
                                         <div class="col-md-3">
                                             <div class="input-group input-group-sm">
                                                <div class="input-group-prepend"><span class="input-group-text small font-weight-bold" style="font-size: 9px;">TRANS</span></div>
                                                <input type="text" class="form-control form-control-sm shipVal font-weight-bold" name="shipping" placeholder="0" onkeypress="return isNumber(event)" onkeyup="billUpyog()" value="<?php echo $invoice['shipping']; ?>">
                                            </div>
                                         </div>
                                    </div>
                                </td>
                            ...
                            <tr class="bg-lighten-5 border-top">
                                <td colspan="4" class="text-right align-middle p-1">
                                    <div class="d-flex justify-content-end align-items-center">
                                        ...
                                        <!-- CHANGED: Populate from $invoice -->
                                        <span class="font-weight-bold text-success" style="font-size: 14px;">TOTAL: <span id="bigtotal"><?= amountExchange_s($invoice['total'], $invoice['multi'], $this->aauth->get_user()->loc); ?></span></span>
                                        <!-- Hidden Inputs Re-added here to ensure JS finds them -->
                                        <input type="hidden" name="total" id="invoiceyoghtml" value="<?= amountExchange_s($invoice['total'], $invoice['multi'], $this->aauth->get_user()->loc); ?>">
                                        <input type="hidden" value="<?= amountExchange_s($invoice['total'], $invoice['multi'], $this->aauth->get_user()->loc); ?>" name="old_total" id="old_invoiceyoghtml">
                                    </div>
                                </td>
                                <td colspan="3" class="p-1">
                                      <a href="#" class="possubmit3 btn btn-success btn-block shadow-sm font-weight-bold py-1" data-type="6" data-toggle="modal" data-target="#basicPay" style="font-size: 14px;">
                                          <i class="fa fa-money mr-1"></i> UPDATE
                                      </a>
                                </td>
                            </tr>
                        </tfoot>
                    ...
                    <input type="text" id="invoiceyoghtmlplaning" name="planing" class="form-control d-none">


                <hr class="my-1">
                <ul class="nav nav-tabs" role="tablist" style="font-size: 11px;">
                    <li class="nav-item">
                        <a class="btn btn-outline-primary btn-sm mr-1 mb-1 py-0 px-2" id="base-tab1" data-toggle="tab"
                    ...
                    <li class="nav-item">
                        <a class="btn btn-outline-danger btn-sm mr-1 mb-1 py-0 px-2" id="base-tab3" data-toggle="tab"
                           aria-controls="tab3" href="#tab3" role="tab" aria-selected="false"><i
                                    class="fa fa-save"></i> <?php echo $this->lang->line('Draft') ?></a>
                    </li>
                    <li class="nav-item">
                    ...
                    <div class="tab-pane" id="tab1" role="tabpanel" aria-labelledby="base-tab1">
                        <div class="input-group input-group-sm">
                            <input type="text" class="form-control form-control-sm"
                                   id="coupon" name="coupon" placeholder="Coupon Code"><input type="hidden"
                                                                    name="coupon_amount"
                                                                    id="coupon_amount"
                                                                    value="0"><span
                                    class="input-group-addon round p-0"> <button
                                        class="apply_coupon btn btn-sm btn-primary sub-btn py-1 px-2"><?php echo $this->lang->line('Apply') ?></button></span>
                        </div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <table class="table table-striped table-bordered">
                    <tr><td>Alt+X</td><td>Focus to products search</td></tr>
                    <tr><td>Alt+C</td><td>Focus to customer search</td></tr>
                    <tr><td>Alt+S (twice)</td><td>PayNow + Thermal Print</td></tr>
                    <tr><td>Alt+Z</td><td>Make Card Payment</td></tr>
                    <tr><td>Alt+Q</td><td>Select First product</td></tr>
                    <tr><td>Alt+N</td><td>Create New Invoice</td></tr>
                </table>
            </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-5 col-md-5 pull-right">
                                <div class="form-group text-bold-600 text-g">
                                    ...
                                    <input type="text" class="form-control green" onkeypress="return isNumber(event)" name="b_change" id="change_p" value="0">
                                </div>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col">
                                <button class="btn btn-success btn-lg btn-block" type="submit" id="pos_basic_pay" data-type="4"><i class="icon icon-arrow-circle-o-right"></i> <?php echo $this->lang->line('Paynow') ?>
                                </button>
                            </div>
                        </div>
                ...
                <div class="row">
                    <div class="col-6">
                        <div class="form-group  text-bold-600 green">
                            <label for="amount"><?php echo $this->lang->line('Cash') ?> (<?= $this->config->item('currency'); ?>)</label>
...
<script type="text/javascript">
    ...
    $('#pos_basic_pay').on("click", function (e) {
        ...
        // Check if old_product_qty exists
        var oldQtyInputs = $('input[name="old_product_qty[]"]');
        console.log('Old Qty Inputs Count:', oldQtyInputs.length);
        oldQtyInputs.each(function(index) {
            console.log('  Old Qty ' + index + ':', $(this).val());
        });
        
        // Check if old_product_pquick exists  
        var oldPquickInputs = $('input[name="old_product_pquick[]"]');
        console.log('Old Pquick Inputs Count:', oldPquickInputs.length);
        oldPquickInputs.each(function(index) {
            console.log('  Old Pquick ' + index + ':', $(this).val());
        });
        
        // Check current quantities
        var qtyInputs = $('input[name="product_qty[]"]');
        console.log('Current Qty Inputs Count:', qtyInputs.length);
        qtyInputs.each(function(index) {
            console.log('  Current Qty ' + index + ':', $(this).val());
        });
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        // Injected Handler for removeItem/removeProd (matches newinvoice_v2 style)
        $('#pos_items').on('click', '.removeItem, .removeProd', function () {
            var pidd = $(this).closest('tr').find('.pdIn').val();
            var pcode = $(this).closest('tr').find('.product_quick_code').val();
            var pqty = $(this).closest('tr').find('.amnt').val();
            var pqty2 = $(this).closest('tr').find('.o_pquick').val();

            if(pidd && pqty) {
                var pqty_val = pidd + '-' + pqty;
                $('<input>').attr({
                    type: 'hidden',
                    id: 'restock',
                    name: 'restock[]',
                ...
                var pqty3_val = pidd + '-' + pqty2;
                $('<input>').attr({
                    type: 'hidden',
                    id: 'restock',
                    name: 'restock_cubic_3[]',
                    value: pqty3_val // or use pqty2 if you want the original quantity
                }).appendTo('form');

                var pqty2_val = pcode + '-' + pqty2;
                $('<input>').attr({
                    type: 'hidden',
                    id: 'restock',
                    name: 'restock_cubic[]',
                    value: pqty2_val // or use pqty2 if you want the original quantity
                }).appendTo('form');
            }
