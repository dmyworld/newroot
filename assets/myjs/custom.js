...
$('#addproduct_secondary').on('click', function () {
    var cvalue = parseInt($('#ganak3').val()) + 1;
    var nxt = parseInt(cvalue);
    $('#ganak3').val(nxt);
    var functionNum = "'" + cvalue + "'";
    ...
    var data = '<tr>' +
        '<td>' +
        '<input type="text" class="form-control" name="product_name2[]" placeholder="Enter Product name or Code" id="productname3-' + cvalue + '">' +



        '</td>' +
        '<td>' +
        '<input type="text" class="form-control req amnt" name="product_qty[]" id="amountt-' + cvalue + '" onkeypress="return isNumber(event)" onkeyup="calculateQuick(' + cvalue + '), rowTotal(' + functionNum + '), billUpyog()" autocomplete="off" value="1">' +
        '</td>' +
        '<td>' +
        '</td>' +
        '<td>' +
        '</td>' +
        '<td>' +
        '<span class="currenty">Total Cubic :</span> <strong><span class="woodttlText" name="result" id="result3wood-' + cvalue + '">0</span></strong>' +
        '</td>' +
        ...
        '<input type="hidden" name="product_description[]" id="dpid-' + cvalue + '" value="">' +
        '<input type="hidden" name="product_pwith[]" id="product_pwith-' + cvalue + '" value="">' +
        '<input type="hidden" name="product_thicknes[]" id="product_thicknes-' + cvalue + '" value="">' +
        '<input type="hidden" name="product_quick[]" id="totalquick-' + cvalue + '" value="">' +
        '<input type="hidden" name="product_quick_code[]" id="product_quick_code-' + cvalue + '" value="">' +
        '<input type="hidden" name="pquick[]" id="pquick-' + cvalue + '" value="">' +
        '<input type="hidden" name="tpquick[]" id="tpquick-' + cvalue + '" value="">' +
    ...
    $('#productname3-' + cvalue).autocomplete({
        source: function (request, response) {
            var billtype = $('#billtype2').val(); // Use billtype2 ('search_logs_wood')
            $.ajax({
                url: baseurl + 'search_products/' + billtype,
                dataType: "json",
                method: 'post',
                data: 'name_startsWith=' + request.term + '&type=product_list&row_num=' + row + '&wid=' + $("#s_warehouses option:selected").val() + '&product_quick_code=' + $('#product_quick_code-0').val() + '&' + d_csrf,
                success: function (data) {
                    response($.map(data, function (item) {
                        var product_d = item[0];
                        return {
        ...
        select: function (event, ui) {
            ...
            $('#product_pwith-' + id[1]).val(ui.item.data[9]);
            $('#product_thicknes-' + id[1]).val(ui.item.data[10]);
            var pquickVal = (ui.item.data[11] && ui.item.data[11] != 0) ? ui.item.data[11] : ui.item.data[10];
            $('#pquick-' + id[1]).val(pquickVal);
            $('#product_quick_code-' + id[1]).val(ui.item.data[12]);
            $('#result2-' + id[1]).val(0); // Explicitly set to 0 as 'search' doesn't return result2

            calculateQuick(cvalue);
            rowTotal(cvalue);
            billUpyog();
...
// Autocomplete for the first row (static)
$('#productname-0').autocomplete({
    source: function (request, response) {
        ...
        $.ajax({
            ...
            dataType: "json",
            method: 'post',
            data: 'name_startsWith=' + request.term + '&type=product_list&row_num=0&wid=' + $("#s_warehouses option:selected").val() + '&' + d_csrf,
            success: function (data) {
                response($.map(data, function (item) {
                    var product_d = item[0];
                    return {
...
$('#addproduct2').on('click', function () {
    ...
    //product row
    var data = '<tr>' +
        '<td><input type="text" class="form-control text-center" name="product_name2[]" placeholder="Enter Product name" id="productname-' + cvalue + '"></td>' +
        '<td><input type="text" class="form-control req amnt" name="product_qty[]" id="amount-' + cvalue + '" onkeypress="return isNumber(event)" onkeyup="calculateQuickLogs(' + functionNum + '), rowTotal(' + functionNum + '), billUpyog()" autocomplete="off" value="1"></td>' +
        '<td><input type="text" class="form-control req prc" name="product_price[]" id="price-' + cvalue + '" onkeypress="return isNumber(event)" onkeyup="calculateQuickLogs(' + functionNum + '), rowTotal(' + functionNum + '), billUpyog()" autocomplete="off"></td>' +
        '<td><input type="text" class="form-control req prc" name="product_r[]" id="r-' + cvalue + '" onkeypress="return isNumber(event)" onkeyup="calculateQuickLogs(' + functionNum + '), rowTotal(' + functionNum + '), billUpyog()" autocomplete="off"></td>' +
        '<td><input type="text" class="form-control req prc" name="product_l[]" id="l-' + cvalue + '" onkeypress="return isNumber(event)" onkeyup="calculateQuickLogs(' + functionNum + '), rowTotal(' + functionNum + '), billUpyog()" autocomplete="off"></td>' +
        '<td><span class="currenty">' + currency + ':</span> <strong><span class="ttlText" id="result1-' + cvalue + '">0</span></strong><br><span class="currenty">ft&#179; :</span> <strong><span class="ttlTextt" id="result2-' + cvalue + '">0</span></strong></td>' +
        '<td class="text-center"><button type="button" data-rowid="' + cvalue + '" class="btn btn-danger removeProd" title="Remove"><i class="fa fa-minus-square"></i></button>' +
        '<input type="hidden" name="taxa[]" id="taxa-' + cvalue + '" value="0">' +
        '<input type="hidden" name="disca[]" id="disca-' + cvalue + '" value="0">' +
        '<input type="hidden" class="ttInput" name="product_subtotal[]" id="total2-' + cvalue + '" value="0">' +
        '<input type="hidden" class="ttInput" name="product_quick[]" id="totalquick_logs-' + cvalue + '" value="0">' +
        '<input type="hidden" class="pdIn" name="pid[]" id="pid-' + cvalue + '" value="0">' +
        '<input type="hidden" name="unit[]" id="unit-' + cvalue + '" value="">' +
        '<input type="hidden" name="hsn[]" id="hsn-' + cvalue + '" value="">' +
        '</td>' +
...
function calculateQuick(rowNumber) {
    ...
    var amountt = parseFloat($('#amountt-' + rowNumber).val()) || 0;
    var price = parseFloat($('#price-' + rowNumber).val()) || 0;

    var pquick = parseFloat($('#pquick-' + rowNumber).val()) || 0;

    var qty = amount + amountt;
    var productpquick = pquick * qty;

    // Formula: (Round * Round * Length) / 2304
    var cubicFeet = (product_round_inches * product_round_inches * product_long_feet) / 2304;
    ...
    $('#result3wood-' + rowNumber).text(productpquick.toFixed(2));
...
function calculateQuickLogs(rowNumber) {
    var product_round_inches = accounting.unformat($('#r-' + rowNumber).val(), accounting.settings.number.decimal) || 0;
    var product_long_feet = accounting.unformat($('#l-' + rowNumber).val(), accounting.settings.number.decimal) || 0;
    var amount = accounting.unformat($('#amount-' + rowNumber).val(), accounting.settings.number.decimal) || 0;
    var price = accounting.unformat($('#price-' + rowNumber).val(), accounting.settings.number.decimal) || 0;

    // Formula: (Round * Round * Length) / 2304
    var cubicFeet = (product_round_inches * product_round_inches * product_long_feet) / 2304;
    var productInFeet = amount * cubicFeet;

    var productSubtotal = productInFeet * price;

    $('#result2-' + rowNumber).text(productInFeet.toFixed(2));
    $('#result1-' + rowNumber).text(productSubtotal.toFixed(2));

    $('#total2-' + rowNumber).val(productSubtotal.toFixed(2));
    $('#totalquick_logs-' + rowNumber).val(productInFeet.toFixed(2));
...
$('#addproduct5').on('click', function () {
    ...
    //product row
    var data = '<tr>' +
        '<td><input type="text" class="form-control text-center" name="product_name[]" placeholder="Enter Product name" id="productname-' + cvalue + '"></td>' +
        '<td><input type="text" class="form-control req amnt" name="product_qty[]" id="amount-' + cvalue + '" onkeypress="return isNumber(event)" onkeyup="calculateQuicktreatment(' + functionNum + '), rowTotal(' + functionNum + '), billUpyog()" autocomplete="off" value="1"></td>' +
        ...
        '<td><input type="text" class="form-control req prc" name="product_t[]" id="t-' + cvalue + '" onkeypress="return isNumber(event)" onkeyup="calculateQuicktreatment(' + functionNum + '), rowTotal(' + functionNum + '), billUpyog()" autocomplete="off"></td>' +
        '<td><input type="text" class="form-control req prc" name="product_w[]" id="w-' + cvalue + '" onkeypress="return isNumber(event)" onkeyup="calculateQuicktreatment(' + functionNum + '), rowTotal(' + functionNum + '), billUpyog()" autocomplete="off"></td>' +
        '<td><input type="text" class="form-control req prc" name="product_l[]" id="l-' + cvalue + '" onkeypress="return isNumber(event)" onkeyup="calculateQuicktreatment(' + functionNum + '), rowTotal(' + functionNum + '), billUpyog()" autocomplete="off"></td>' +
        '<td><span class="currenty">' + currency + ':</span> <strong><span class="ttttlText" id="resultttr-' + cvalue + '">0</span></strong><br><span class="currenty">ft&#179; :</span> <strong><span class="ttlTextt" id="resultttc-' + cvalue + '">0</span></strong></td>' +
        ...
        '<input type="hidden" name="taxa[]" id="taxa-' + cvalue + '" value="0">' +
        '<input type="hidden" name="disca[]" id="disca-' + cvalue + '" value="0">' +
        '<input type="hidden" class="ttInput" name="product_subtotal[]" id="totaltt1-' + cvalue + '" value="0">' +
        '<input type="hidden" class="ttInput" name="product_quick[]" id="totalquick-' + cvalue + '" value="0">' +
        '<input type="hidden" class="pdIn" name="pid[]" id="pid-' + cvalue + '" value="0">' +
        '<input type="hidden" name="unit[]" id="unit-' + cvalue + '" value="">' +
        '<input type="hidden" name="hsn[]" id="hsn-' + cvalue + '" value="">' +
        '</tr>'; //ajax request
...
$('#addproduct').on('click', function () {
    ...
    if (flag) {
        ...
        var unit_val = $(this).attr('data-unit');
        var input_html;
        if (unit_val && (unit_val.trim().toLowerCase() == 'feet' || unit_val.trim().toLowerCase() == 'sqft' || unit_val.trim().toLowerCase() == 'sq.ft')) {
            input_html = '<input type="text" class="form-control p-mobile req prc" name="product_rate[]" id="rate-' + cvalue + '" onkeypress="return isNumber(event)" onkeyup="rowTotal(' + functionNum + '), billUpyog()" autocomplete="off" value="' + $(this).attr('data-rate') + '"><input type="hidden" name="product_price[]" id="price-' + cvalue + '" value="' + $(this).attr('data-price') + '">';
        } else {
            input_html = '<input type="text" class="form-control p-mobile req prc" name="product_price[]" id="price-' + cvalue + '" onkeypress="return isNumber(event)" onkeyup="rowTotal(' + functionNum + '), billUpyog()" autocomplete="off" value="' + $(this).attr('data-price') + '"><input type="hidden" name="product_rate[]" id="rate-' + cvalue + '" value="' + $(this).attr('data-rate') + '">';
        }
...
var o_rowTotal = function (numb) {
    ...
    var rateVal = formInputGet("#price", numb);
    ...
    var taxo = 0;
    var disco = 0;
    var totalPrice = (parseFloat(amountVal).toFixed(2)) * rateVal;
...
var rowTotal = function (numb) {
    ...
    var rateVal = accounting.unformat($("#rate-" + numb).val(), accounting.settings.number.decimal);
    ...
    if (unitVal == 'Sqft') {
        ...
        totalPrice = amountVal.toFixed(two_fixed) * rateVal * sqftVal;

    } else if (unitVal == 'Feet') {
        totalPrice = amountVal.toFixed(two_fixed) * rateVal * length_val;
    } else {
    ...
    // SQFT Calculation
    var prod_pwith = parseFloat($("#product_pwith-" + numb).val()) || 0;
    // Assuming product_description holds length or similar dimension as per user request
    // Sometimes description is text, but user specifically asked for product_description * pwith * qty.
    // However, if description field is text, this might be NaN. 
    // Checking previous context, user used data attributes for these.
    // Let's use the input values.
    var prod_desc_val = parseFloat($("#product_description-" + numb).val()) || 0;
    var prod_qty_val = parseFloat($("#amount-" + numb).val()) || 0;

    // User requested formula: pwith * description * qty
    // (Usually this involves unit conversion /12 or /144 but I will stick to exact user request multiplication first, 
    // or checks standard cubic feet logic: (width_inch * thickness_inch * length_feet) / 144 ??
    // User specifically asked: ( product_pwith[] * product_description[] * product_qty[] ).
    // I will simply multiply them. If they want /144 they usually say "cubic". "sqft" usually implies width*length.
    // If pwith is inches and description is feet, to get sqft: (width/12) * length
    // If both are feet: width * length
    // I will implement: (pwith * description * qty) and assume units are consistent or user handles it. 
    // Wait, typically pwith is inches. If so, it should be /12. 
    // Let's look at previous code: `sub_qty2 = $row['pwith']/12*$row['product_des'] * $row['qty'];` in view.php suggests /12.
    // I will add /12 to be safe for SQFT if pwith is inches.
    // HOWEVER, user's PROMPT just said: `product_pwith[] * product_description[] * product_qty[]`. 
    // I will use `(prod_pwith * prod_desc_val * prod_qty_val) / 12` 
    // Actually, looking at the view.php again: `$sub_qty2 = $row['pwith']/12*$row['product_des'] * $row['qty'];`
    // This matches the view logic. 
    var total_sqft = (prod_pwith / 12) * prod_desc_val * prod_qty_val;
    $("#sqft-" + numb).val(total_sqft.toFixed(2));

    // Cubic Feet (o_pquick) Calculation
    // Formula: (pwith * thickness * description * qty) / 144
    // pwith is width in inches.
    // thickness is in inches.
    // description is length in feet.
    // Result is Cubic Feet.
    var prod_thick_val = parseFloat($("#product_thicknes-" + numb).val()) || 0;

    // Explanation: (Width_in * Thick_in * Length_ft) / 144  ==  (Width_in/12) * (Thick_in/12) * Length_ft * 12 ?? No.
    // 1 Board Foot = 144 cubic inches. (1 ft x 1 ft x 1 in). Definition: W_in * L_ft * T_in / 12.
    // User asked for "Total Cubic Feet". 
    // 1 Cubic Foot = 1728 cubic inches.
    // (W_in * T_in * L_ft) / 144:
    // = (W_in * T_in * L_in/12) / 144 
    // = (Vol_in3 / 12) / 144 = Vol_in3 / 1728 = Vol_ft3.
    // So YES, dividing by 144 (when using inches, inches, feet) GIVES CUBIC FEET.

    var total_cubic_feet = (prod_pwith * prod_thick_val * prod_desc_val * prod_qty_val) / 144;
    $("#o_pquick-" + numb).val(total_cubic_feet.toFixed(2));
    if ($('#result3wood-' + numb).length === 0) {
        $("#pquick-" + numb).val(total_cubic_feet.toFixed(2));
    }
