...
$(document).ready(function () {
    ...
    $('#warehouses').change(function () {
        ...
        var sub_cat = $('#sub_categories option:selected').val();
        $.ajax({
            type: "POST",
            url: baseurl + 'search_products/pos_search',
            data: 'wid=' + whr + '&cid=' + cat + '&sub_id=' + sub_cat + '&' + crsf_token + '=' + crsf_hash,
            beforeSend: function () {
                $("#customer-box").css("background", "#FFF url(" + baseurl + "assets/custom/load-ring.gif) no-repeat 165px");
            },
            success: function (data) {
    ...
    $('#categories').change(function () {
        ...
        $.ajax({
            ...
            data: 'id=' + cat + '&' + crsf_token + '=' + crsf_hash,
            success: function (data) {
                $('#sub_categories').html(data);
                // Reset select match to 0
    ...
    $('#search_bar').keyup(function () {
        ...
        $.ajax({
            type: "POST",
            url: baseurl + 'search_products/pos_search',
            data: 'name=' + $(this).val() + '&wid=' + whr + '&cid=' + cat + '&sub_id=' + sub_cat + '&' + crsf_token + '=' + crsf_hash,
            beforeSend: function () {
                $("#customer-box").css("background", "#FFF url(" + baseurl + "assets/custom/load-ring.gif) no-repeat 165px");
            },
            success: function (data) {
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.status == 'Success') {
                ...
                $("html, body").animate({
                    scrollTop: $('#thermal_a').offset().bottom
                }, 1000);
            } else {
...
$(document).on('click', "#purchasepayment_wood", function (e) {
    e.preventDefault();

    var pyurl = baseurl + 'transactions/paypurchase_wood';
...
function payInvoice(pyurl) {
    ...
    if (errorNum > 0) {
        ...
        $("html, body").animate({
            scrollTop: $('#notify').offset().bottom
        }, 1000);
    } else {
        jQuery.ajax({
